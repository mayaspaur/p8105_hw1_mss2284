...
The dataset has a number of rows`r nrow(pull(problem2_df))`, and number of columns `r ncol(pull(problem2_df))`.
...
The proportion of cases for which x + y is greater than one is `r sum(pull(problem2_df, vec_logical_2))/sum(pull(problem2_df, vector_sum))`.
...
```
 ...
 For the scatterplot with the as.numeric variable, the color scale is continuous from light to dark blue to reflect the continuous variable. For the plot using the logical variable and factor variable, there were two colors, to represent the binary (logical) variable, blue for true and orange for false.